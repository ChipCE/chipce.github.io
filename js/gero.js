...
function make()
{
    ...
    if(level == "main")
    {
        ...
        else if(type=="checkbox")
        {
            let value = $("#mainDefaultValue").val();
            console.log(value);
            let configStr = '<!-- @' + name + ' -->\n<div class="config-option-warpper col-12">\n\t<input class="form-check-input config-option" type="checkbox" value="" id="' + name + '" name="' + name + '" autocomplete="off"' + ((value=="true")?' checked="checked"':'') +'>\n\t<label class="form-check-label text-primary" for="' + name + '">' + diplayName + '</label>\n\t<p class="text-muted"><i class="fa fa-info-circle"></i> ' + description + '</p>\n</div>';
            $("#source").text(configStr);
        }
        else if(type=="select")
        {
            ...
            console.log(argList);
            let configStr='<!-- @' + name + ' -->\n<div class="config-option-warpper col-12">\n\t<label for="' + name + '" class="text-primary"><i class="fa fa-chevron-down"></i> ' + diplayName + '</label>\n\t<select class="custom-select config-option" name="' + name + '" autocomplete="off" id="' + name + '" appendId="' + appendId + '">\n';
            for(let i = 0;i<argList.length;i++)
            {
                
                configStr = configStr + '\t\t<option value="' + argList[i] + '"' + ((i==0)?' selected="selected"':'') +'>' + argList[i] +'</option>\n';
            }
            configStr = configStr + '\t</select>\n\t<p class="text-muted"><i class="fa fa-info-circle"></i> ' + description + '</p>\n</div>';
            $("#source").text(configStr);
        }
    }
    else
